...
type RestorableGame = {
  playbackConfig?: import("@/lib/game/playback-config").GamePlaybackConfig | null;
  id: string;
  joinCode: string;
  playlistId: string;
  playlistName: string;
  ...
  useEffect(() => {
    ...
    async function restoreGame() {
      try {
        ...
        const response =
          await fetch(
            `/api/game/restore?gameId=${encodeURIComponent(
              gameId
            )}`,
            {
              cache:
        ...
        if (game.id !== gameId) throw new Error("The saved game did not match your selection.");
        const config = game.playbackConfig;
        if (!config) throw new Error("This older game is missing its music source. Create a new game from your music library to restore audio playback.");
        if (
          game.tracks.length ===
          0
        ) {
        ...
        const restoredTracks:
          ...
          game.tracks.map(
            (track) => ({
              ...config.tracks.find((savedTrack) => savedTrack.id === track.id),
              id:
                track.id,
              gameTrackId:
                track.gameTrackId,
          ...
          GameSession = {
            ...
            status:
              game.status ===
                "completed"
                ? "complete"
                : game.status === "waiting" ? "ready" : "paused",
            tracks:
              restoredTracks,
            playedTrackIds:
              game.calledTrackIds,
        ...
        if (!controller.signal.aborted) setIsRestoringGame(false);
      }
    }
    void restoreGame();
    return () => controller.abort();
    // A new game ID remounts this console; playback state must not restart loading.
        ...
        <section className="dj-access-strip">
          <div className="dj-access-panel">
            ...
            <section style={{ padding: 16 }}>
              <Link href="/dashboard">Choose a Saved Game</Link>
              {isRestoringGame && <p role="status">Loading your saved game...</p>}
            </section>
              ...
              <section className="dj-panel">
                ...
                {session.source === "apple" && <>
                  ...
                  <button type="button" className="dj-primary-button" onClick={() => {
                    void getAuthorizedAppleMusic().then(() => {
                      setAppleConnected(true);
                      setMessage("Apple Music connected. Ready to play.");
                    }).catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Unable to connect Apple Music."));
              ...
              <div className="dj-control-grid">
                <button
                  ...
                  onClick={() => {
                    ...
                    setMessage(isPlaying ? "Pausing playback..." : "Starting playback...");
                    void updateStatus(isPlaying ? "paused" : "playing");
                  }}
                >
                  <b>{isPlaying ? "Ⅱ" : "▶"}</b>
