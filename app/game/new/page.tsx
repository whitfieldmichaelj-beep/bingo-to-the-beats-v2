...
export default function CreateGamePage() {
  ...
  return <main className="billing-page"><div className="billing-container">
    ...
    <section className="host-type-options" aria-label="Choose your host type">
      <button type="button" className={`host-type-card ${hostType === "dj" ? "selected" : ""}`} aria-pressed={hostType === "dj"} onClick={() => selectHost("dj")}><strong>I’m a DJ</strong><span>Play in Serato and run your bingo game here.</span></button>
    ...
    {hostType && <section className="billing-card host-next-step" aria-live="polite"><p className="billing-eyebrow">{hostType === "dj" ? "DJ MUSIC OPTIONS" : "YOUR GAME NIGHT"}</p><h2>{hostType === "dj" ? "Choose the music for your show" : "Connect your streaming service"}</h2><p>{hostType === "dj" ? "Import your song list, then connect your public Serato Live Playlist." : "Choose a playlist from your supported music service and invite your group. No DJ equipment required."}</p><div className="billing-actions">{hostType === "dj" && <><Link className="billing-button" href="/music/upload">Import DJ song list</Link><Link href="/serato">This Mac’s Serato library</Link></>}{hostType === "other" && <Link className="billing-button" href={`/music?host=${hostType}`}>Music options</Link>}</div><Link href={`/pricing?host=${hostType}`}>View weekly &amp; monthly plans →</Link></section>}
